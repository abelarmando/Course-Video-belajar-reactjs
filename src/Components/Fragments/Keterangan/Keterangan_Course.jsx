...
function Keterangan_Course({ data, hide = "hidden" }) {
  ...
  return (
    <div className="border-[1px] border-[#F1F1F1] rounded-md p-6 bg-white w-full space-y-4 h-fit">
      ...
      />
      <h6 className="text-base font-bold">{product.titlepage}</h6>
      <div className="flex justify-between items-center">
        <div className="space-x-2 flex">
          <p className="text-green-400 text-lg font-bold">{product.tagprice}</p>
      ...
      <ul className="grid grid-cols-2 gap-5">
        {product.length == undefined &&
          product.courseinclude.map((item, i) => (
      ...
      <div className="space-y-2">
        ...
        {product.length == undefined &&
          product.courselanguage.map((item, i) => (
